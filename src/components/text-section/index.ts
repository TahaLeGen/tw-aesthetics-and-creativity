...
export class TextSectionSplitLayout extends LitElement {
  ...
  private get cfg() {
    return {
      ...
      section_title:    localizedString(this.config?.['section_title'],    'Building the Future Infrastructure'),
      section_subtitle: localizedString(this.config?.['section_subtitle'], 'SCALABLE ARCHITECTURE FOR MODERN APPLICATIONS'),
      main_paragraph:   localizedString(this.config?.['main_paragraph'],   'Our core framework is designed from the ground up to solve complex distributed systems challenges seamlessly.'),
      ...
      ]) as { title: string | Record<string, string>; description: string | Record<string, string> }[],
    };
  }
  ...
  render() {
    ...
    return html`
      <style>
        .text-layout-section { background:${c.background}; padding:6rem 1.5rem; color:${c.text_color}; font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif; }
      ...
      <section class="text-layout-section" dir="rtl">
        <div class="text-layout-inner">
          <div style="text-align:center;max-width:48rem;margin:0 auto 4rem;" data-sal="slide-up" data-sal-duration="800">
            <span style="display:block;font-size:0.75rem;font-weight:600;letter-spacing:0.15em;text-transform:uppercase;color:${c.accent_color};margin-bottom:0.75rem;">${c.section_subtitle}</span>
            <h2 style="margin:0;font-size:clamp(1.875rem,4vw,3rem);font-weight:700;letter-spacing:-0.025em;color:${c.text_color};line-height:1.15;">${c.section_title}</h2>
          </div>
          <div class="text-layout-grid">
            <div data-sal="slide-up" data-sal-duration="800" data-sal-delay="100">
              <p style="margin:0;font-size:1.125rem;color:rgba(255,255,255,0.75);font-weight:400;line-height:1.8;text-align:justify;">${c.main_paragraph}</p>
            </div>
            <div style="display:flex;flex-direction:column;gap:1.25rem;">
              ${c.side_blocks.map((block, index) => html`
                <div class="glass-info-block" data-sal="slide-up" data-sal-duration="700" data-sal-delay="${200 + index * 100}">
                  <h3 style="margin:0 0 0.5rem;font-size:1rem;font-weight:600;color:${c.text_color};line-height:1.4;">${localizedString(block.title)}</h3>
                  <p style="margin:0;font-size:0.9rem;color:rgba(255,255,255,0.55);font-weight:400;line-height:1.7;">${localizedString(block.description)}</p>
                </div>
              `)}
            </div>
          </div>
