...
export class MainLinksStyle1 extends LitElement {
  ...
  private get cfg() {
    return {
      section_title:           localizedString(this.config?.['section_title'],        'Explore Our Global Network'),
      section_description:     localizedString(this.config?.['section_description'],  'Discover specialized hubs, regional ecosystems, and open-source documentation.'),
  ...
  private renderLinkCard(link: LinkItem, index: number) {
    const title = localizedString(link.title);
    return html`
      <div class="swiper-slide" style="height:auto;flex-shrink:0;box-sizing:border-box;">
        <a href="${link.link}" class="ml-card" data-sal="slide-up" data-sal-duration="700" data-sal-delay="${(index + 1) * 80}"
          ...
          <div style="width:100%;aspect-ratio:4/3;overflow:hidden;">
            <img loading="lazy" src="${link.image}" alt="${title}" style="width:100%;height:100%;object-fit:cover;display:block;transition:transform 0.5s ease;">
          </div>
          <div style="display:flex;align-items:center;justify-content:space-between;padding:14px 16px;gap:8px;">
            <h4 style="margin:0;font-size:0.9375rem;font-weight:500;color:#ffffff;line-height:1.4;">${title}</h4>
  ...
  render() {
    ...
    return html`
      <style>
        .ml-section { position:relative; padding:5rem 0; overflow:hidden; background:${c.background}; border-top:1px solid rgba(255,255,255,0.07); border-bottom:1px solid rgba(255,255,255,0.04); }
